  ...
  return (
    <div className="flex flex-row items-start p-4 gap-8">
      <div>
        ...
        <div className="grid gap-1" style={{gridTemplateColumns: `repeat(${grid}, 40px)`,  gridTemplateRows: `repeat(${grid}, 40px)`, }}>
          {Array.from({ length: grid }).map((_, row) =>
            Array.from({ length: grid }).map((_, col) => {
              ...
              return (
                <div
                  key={`${row}-${col}`} className={`w-10 h-10 border flex items-center justify-center ${isBot  ? "bg-blue-800" : isProbe ? "bg-green-500" : isArmOnProbe ? "bg-green-300" : isArm ? "bg-blue-400" : "bg-gray-200" }`}
                ></div>
              );
            })
          )}
        </div>
        <input type="text" placeholder="Harakat: w, a, s, d | Olish: k | Qo'yish: l" className="p-2 border rounded mt-4 w-full" onChange={handleInput} />
      </div>
        ...
        <div className="max-h-[500px] overflow-y-auto p-2 border rounded bg-gray-50">
          {history.map((session, index) => (
            <div key={index} className="p-4 border-b mb-2">
              <p><strong>Boshlanish vaqti:</strong>{" "} {session.start ? new Date(session.start).toLocaleTimeString() : ""} </p>
              <p><strong>Tugash vaqti:</strong>{" "} {session.end ? new Date(session.end).toLocaleTimeString() : ""} </p>
              <p> <strong>Bosilgan tugmalar:</strong> {session.keys.join(", ")} </p>
            </div>
          ))}
        </div>
      </div>
